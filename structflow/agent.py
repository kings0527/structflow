...
V2.1 Core Principle:
  ...
  Any industry is compressed into: SV + FV + CV + LV.
"""
...
def run_scan(
    ...
    Pipeline: Search → L0 → [search] → L1 → [search] → L2 → [search] → L3 → [search] →
              L4 → [search] → L5 → [search] → L6 → [search] → L7(optional) →
              Gates → Output
    """
    if client is None:
    ...
    console.print(f"  Function: {l0_result.core_function}")
    ...
    if enable_challenge:
        try:
            l2_result = challenge_l2(client, scan_input.industry, l2_result, l1_result, context_data=context_data)
        except Exception as error:
    ...
    if enable_challenge:
        try:
            l3_result = challenge_l3(client, scan_input.industry, l3_result, l1_result, context_data=context_data)
        except Exception as error:
    ...
    if enable_challenge:
        try:
            l4_result = challenge_l4(client, scan_input.industry, l4_result, l3_result, context_data=context_data)
        except Exception as error:
    ...
    l5_result = retry_guard.run_with_retry(
        func=lambda **kw: run_l5(client, scan_input, l0_result, l1_result, l2_result, l3_result, l4_result, context_data=context_data, **kw),
        validate_func=lambda result: [validator.validate_distortion(result)],
        layer_name="L5",
    )
    if enable_challenge:
        try:
            l5_result = challenge_l5(client, scan_input.industry, l5_result, l4_result, context_data=context_data)
        except Exception as error:
    ...
    l6_result = retry_guard.run_with_retry(
        func=lambda **kw: run_l6(client, scan_input, l0_result, l1_result, l2_result, l3_result, l4_result, l5_result, context_data=context_data, **kw),
        validate_func=lambda result: [validator.validate_alpha_completeness(result)],
        layer_name="L6",
    )
    if enable_challenge:
        try:
            l6_result = challenge_l6(client, scan_input.industry, l6_result, l5_result, context_data=context_data)
        except Exception as error:
    ...
    console.print(f"  Structural: {l6_result.structural_view[:80]}...")
...
def _extract_fragilities(l0_result, l4_result, l5_result, l6_result) -> list[str]:
    ...
    # Check for dangerous feedback loops
